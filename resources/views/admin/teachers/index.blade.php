<x-app-layout>
    <div class="p-6">
        <!-- Header -->
        <div class="bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700 rounded-xl p-6 shadow-sm mb-6">
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">👨‍🏫 All Teachers</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400">List of all assigned teachers</p>
                    </div>
                </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold">Madrasah</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
                    @forelse($teachers as $teacher)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            ...
                            <td class="px-4 py-2">{{ $teacher->madrasah->name ?? '-' }}</td>
                            <td class="px-4 py-2">{{ $teacher->designation ?? '-' }}</td>
                            <td class="px-4 py-2">{{ $teacher->subject ?? '-' }}</td>
                            <td class="px-4 py-2 text-center">
                                @if($teacher->status === 'active')
                                    <span class="inline-block px-2 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-full">Active</span>
                                @else
                                    <span class="inline-block px-2 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-full">Inactive</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center px-4 py-4 text-gray-500 dark:text-gray-400">No teachers found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
